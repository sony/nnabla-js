...
import {
  IExecutor, INetwork, IParameter, NNablaProtoBuf,
} from './nnabla_pb';
...
export function unzipNNP(data: Uint8Array): Promise<NNP> {
  return (new JSZip()).loadAsync(data)
    .then(async (zip) => {
      ...
      let version: string = '';
      const versionPromise = zip.file('nnp_version.txt')?.async('string').then((text) => {
        version = text.trim();
      });
      ...
      const networkPromise = zip.file('network.nntxt')?.async('string').then((text) => {
      ...
      const paramPromise = zip.file('parameter.protobuf')?.async('uint8array').then((byteCode) => {
      ...
      return {
        version, networks, parameters, executors,
      };
    });
}
